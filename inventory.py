...
class Inventory():
        ...
        for slot in self.inventory:
            ...
            if slot.itemName == item.itemName:
                if slot.itemCount > delCount:
                    ...
                    print(f'Removed {delCount} of {item.itemName}s from your inventory.')
                elif delCount == slot.itemCount:
                    del self.inventory[slotCount]
                    print(f'Removed all {item.itemName}s from your inventory.')
                else:
                    # ERROR if not enough.
                    print(f'Unable to delete {delCount} {item.itemName}s.  You only have {slot.itemCount}!')
                found = True
                break
            slotCount += 1
        if not(found):
    ...
    def displayInventory(self):
        ...
        itemsPrice = 0.00
        itemsCount = 0
        colWidths = [9, 5, 6, 5]                         # List of length of column headers.
        totalWidth = 0
        ...
        for item in self.inventory:                      # Calculate column widths
            ...
            if colWidths[2]  < len(str(item.itemWeight)):
                colWidths[2] = len(str(item.itemWeight))
